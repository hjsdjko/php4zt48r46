...
class ShangpinxinxiController extends CommonController {
    ...
    public function autoSort2(){
		...
		$token = $this->token();
        $tokens = json_decode(base64_decode($token),true);
        $orderSql = "select goodtype from orders where userid = ".$tokens['id']." and tablename = 'shangpinxinxi' order by addtime desc";
        $orderInfo = table_sql($orderSql);
        $orderType = array();
        if ($orderInfo->num_rows > 0) {
            while ($orderRow = $orderInfo->fetch_assoc()){
                array_push($orderType,"'".$orderRow['goodtype']."'");
            }
        }
        if (count($orderType) > 0) {
            $lists = "select * from shangpinxinxi where shangpinfenlei in (".implode(', ', $orderType).") union all select * from shangpinxinxi where shangpinfenlei not in (".implode(', ', $orderType).")";
        } else {
            $lists = "select * from shangpinxinxi";
        }
    ...
    public function count() {
        ...
        $sql = "select count(*) as count from shangpinxinxi where 1=1 ".$where;
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
            while($row = $result->fetch_assoc()) {
