...
class YonghuController extends CommonController {
    ...
    public function login(){
        ...
        $sql = "select * from `yonghu` where `yonghuming` = '".$username."' and `mima` = '".$password."'";
    ...
    public function register(){
        ...
        $colum = "yonghuming";
        $trues = "select * from `yonghu` where `yonghuming` = '".$postData[$colum]."'";
    ...
    public function resetPass(){
        ...
        $counts = "select * from `yonghu` where yonghuming = '".$username."'";
        $cotte = table_sql($counts);
        if($cotte->num_rows<1){
            exit(json_encode(['code'=>500,'msg'=>"用户名错误。"]));
        }
        $sql = "update yonghu set mima = '123456' where yonghuming = '".$username."'";
        $result = table_sql($sql);
    ...
    public function save(){
        ...
        if (!empty($tmpData)&& isset($tmpData)){
            $postData = json_decode($tmpData,true);
            $yonghuming = $postData['yonghuming'];
            $resultInfo = table_sql("select yonghuming from `yonghu` where `yonghuming` = '".$yonghuming."'");
    ...
    public function update(){
        ...
        $postData = json_decode($tmpData,true);
            $yonghuming = $postData['yonghuming'];
            $id = $postData['id'];
            $resultInfo = table_sql("select yonghuming from `yonghu` where id <> ".$id." and `yonghuming` = '".$yonghuming."'");
            if ($resultInfo->num_rows > 0) {
                exit(json_encode(['code'=>500,'msg'=>"用户名已存在"]));
            }
        $v = array();
    ...
    public function count() {
        ...
        $sql = "select count(*) as count from yonghu where 1=1 ".$where;
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
            while($row = $result->fetch_assoc()) {
